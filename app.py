...
def sample_images(
    ...
    canvas = torch.zeros(n_images, n_channel, img_size, img_size, device=DEVICE)
    return sample_pixels(model, canvas, 0, img_size)
...
def sample_conditional(
    ...
    """
    img_size = get_dataset_config(dataset)["img_size"]
    half = img_size // 2
    canvas = real_image.repeat(n_completions, 1, 1, 1).to(DEVICE)
    canvas[:, :, half:, :] = 0.0
    return sample_pixels(model, canvas, half, img_size)
...
def _run_epoch(
    ...
    for images, _ in train_loader:
        ...
        loss.backward()
        clip_grad_norm_(solver.model.parameters(), max_norm=solver.grad_clip)
    ...
    solver.scheduler.step(test_loss)
...
def run_training(
    ...
    train_loader, test_loader = build_data_loaders(dataset, batch_size, DATASET_ROOT)
    n_channel = get_dataset_config(dataset)["n_channel"]
...
def _page_train() -> None:
    ...
    lr = st.select_slider(
        ...
        value=get_dataset_config(dataset)["default_lr"],
        format_func=lambda x: f"{x:.0e}",
        help="Adam learning rate. The default is pre-set per dataset — reduce if loss spikes.",
    )
